...
RestServer::RestServer(AsyncWebServer *server, Scheduler *scheduler) {
    ...
    server->on("/module/info", HTTP_GET,
    ...
    server->on(
	"/module/setup", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL,
    ...
    server->serveStatic("/plant/info", SPIFFS, "/data.json")
	.setCacheControl("max-age=60");

    server->on(
	"/plant/setup", HTTP_PUT, [](AsyncWebServerRequest *request) {}, NULL,
	[](AsyncWebServerRequest *request, uint8_t *data, size_t len,
           size_t index, size_t total) {
	    char raw_config[len] = "";
	    strcpy(raw_config, (const char *)data);
